...
Django apps that want to use `dj` should follow these principles:

1. Consistency

    Apps should follow a predictable file and metadata structure.

a. An app should have a name.

    This name should contain only lower-case letters and underscores.

b. An app should have one top-level code package with that name.

    The package name should be the same as the app name.
    For example, app "foo" would package all of its code under the "foo" package.

c. An app should have a top-level `setup.py` file.
    ...
    You must include a setup.py file for Dj to work with your application.
    This setup.py file should include a `name` for your app, which should match the name of your primary
    app package.

    This file is generated by `dj init`, but you likely have one if your application already.

d. An app should have a top-level `manage.py` script.

    This manage.py script should be included in the `scripts` directive in `setup.py`.
    `dj init` will set this up for you.

e. An app may have a top-level `requirements.txt` dependency file.
    ...
    The caveat to `requirements.txt` is that pip does not run itself recursively on this file, so libraries
    that want to distribute with other dependencies need to add the setup directives. Dj does not currently support
    adding dependencies in this way, but this can be done manually in setup.py,
    e.g. by trimming and freezing the dependencies in `requirements.txt`.

    This is generated automatically by `dj init` and kept updated by `dj add` and `dj remove`.
...
f. An app may have a top-level `requirements.txt.dev` dependency file.
    ...
    This is generated automatically by `dj init` and kept updated by `dj add --dev` and `dj remove --dev`.

g. The code package of the app should contain `settings.py`, `wsgi.py`, and `urls.py` packages.
    
    These packages are necessary for Django applications to function properly.
    ...
    `urls.py`:
        Server URL routing file.

    They are generated automatically when you run `dj init`.
...
b. App code should be concise. 
    ...
    Avoid writing repetitive actions into code.
