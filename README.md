...
1a. An app MUST have a name.

    This name should contain only lower-case letters and underscores.

1b. An app MUST have only one top-level code package sharing the app's name.
...
1c. An app MUST have a top-level `setup.py` file.
...
1d. An app may have a top-level `manage.py` script.
...
1e. An app may include a top-level `requirements.txt` dependency file.
...
1f. An app may have a top-level `requirements.txt.dev` dependency file.
...
1g. The code package of the app should contain settings.py, wsgi.py, and urls.py packages.
...
1h. An app should be written in a single code style.

    Style should be enforced by a linter (e.g. `flake8`).
    Naming should be kept consistent between different modules.

1i. An app should contain a tests package for all unit and integration tests.
...
2a. An app should only export one package.
...
2c. App structure should be concise.
